...
### Add a particular path to the system path
...
This is useful for importing files that may be hard to reach using the standard
import syntax. For example, suppose that you have a far-away file that contains
some tools you'd like to use. If the file is located at `/path/to/my_tools.py`,
then it can be imported like this:

```python
# first, add the tools file's parent directory to the system path
import sys
sys.path.append("/path/to")

# then import items from the tools file using the standard import syntax
from my_tools import whatever
```

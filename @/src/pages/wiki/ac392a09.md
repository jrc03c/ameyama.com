...
### Seeing progress in `execSync` in realtime

By default, `execSync` only shows its work when it's done. But you can encourage it to display updates in realtime like this:
...
### Adding path aliases for imports
...
```json
{
  "imports": {
    "#root/*": "./*",
    "#foo/*": "./path/to/foo/*",
    "#bar/*": "./some/other/path/to/bar/*"
  }
}
```

Then in some deep file:

```js
import { something } from "#root/some/path"
```

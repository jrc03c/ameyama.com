...
**HINT #1:** Add the `--verbosity:0` flag to turn off extra info printed by the compiler.

**HINT #2:** Add the `-d:release` flag to compile with aggressive optimizations.
...
for kind, path in walkDir("/some/dir"):
  if kind == pcDir:
    echo "DIRECTORY: " & path
...
Use the [`commandLineParams`](https://nim-lang.org/docs/cmdline.html#commandLineParams) function to get a sequence of strings representing command line arguments:
